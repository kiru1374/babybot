...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.online, activity=discord.Game("만든놈 kiru"))
...
async def 들어와(ctx):
    try:
        ...
        try:
            await vc.message.author.voice.channel.connect()
        except:
...
\nt/도움말 -> 뮤직봇의 모든 명령어를 볼 수 있음.
\nt/들어와 -> 뮤직봇을 자신이 속한 채널로 부름.
\nt/나가 -> 뮤직봇을 자신이 속한 채널에서 내보냄.
\nt/URL재생 [노래링크] -> 유튜브URL를 입력하면 뮤직봇이 노래를 틀어줌.
(대기열재생에서는 사용할 수 없음.)
\nt/재생 [노래이름] -> 뮤직봇이 노래를 검색해 틀어줌.
\nt/노래끄기 -> 현재 재생중인 노래를 끔.
t/일시정지 -> 현재 재생중인 노래를 일시정지함.
t/다시재생 -> 일시정지시킨 노래를 다시 재생함.
\nt/지금노래 -> 지금 재생되고 있는 노래의 제목을 알려줌.
\nt/대기열 -> 이어서 재생할 노래대기열을 보여줌.
t/대기열재생 -> 대기열에 추가된 노래를 재생함.
t/대기열초기화 -> 대기열에 추가된 모든 노래를 지움.
\nt/대기열추가 [노래] -> 노래를 대기열에 추가함.
t/대기열삭제 [숫자] -> 대기열에서 입력한 숫자에 해당하는 노래를 지움.""", color = 0x00ff00))
...
async def 재생(ctx, *, msg):
    ...
    try:
        ...
        try:
            ...
            await ctx.send("채널에 유저가 없거나 노래가 재생 준비중입니다.")
...
def title(msg):
    ...
    chromedriver_dir = load_chrome_driver()
